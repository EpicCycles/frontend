...
export const partReadyToUse = (part, persistedPart) => {
  ...
  if (isItAnObject(part) && isItAnObject(persistedPart))
    return Object.keys(persistedPart).some(key => {
...
export const buildPartObject = (partType, partNameWithBrand, brandsLower, bikeBrand) => {
  ...
  const part_name = brandFound
    ? partNameWithBrand.slice(brandFound.brand_name.length).trim()
    : partNameWithBrand;

  return { partType, part_name, brand };
};

export const buildFrameWorkPartDisplay = (
  sections,
  parts,
  showEmptySections,
  showEmptyPartTypes,
  ...
  const sectionsWithParts = sections.map(section => {
    let partsForSection = [];
    section.partTypes
      .map(partType => parts.filter(bikePart => bikePart.partType === partType.id))
      .forEach(partArray => (partsForSection = partsForSection.concat(partArray)));
    return {
      id: section.id,
      name: section.name,
...
export const findPart = (parts, partType, brand, partName) => {
  const partNameToFind = partName.toLowerCase();
  const partTypeToFind = partType.toString();
  const brandToFind = brand.toString();
  return parts.find(part => {
    return (
      part.partType.toString() === partTypeToFind &&
      part.brand.toString() === brandToFind &&
      part.part_name.toLowerCase() === partNameToFind
    );
  });
};
