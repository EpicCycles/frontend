...
import { QUOTE_ARCHIVED, QUOTE_INITIAL } from '../../../components/quote/helpers/quote';
...
describe('QuoteIssue', () => {
  ...
  it('should render bike parts and section details when a bike is present on quote', () => {
    const component = shallow(
      <QuoteIssue
        quoteId={1}
        quotes={[{ id: 1, bike: 58, quote_status: QUOTE_INITIAL }]}
        quoteParts={[{ quote: 1, partType: 1 }, { quote: 1, partType: 22 }]}
        brands={sampleBrands}
        sections={sections}
  ...
  it('should render redirect when a quote is not new', () => {
    const component = shallow(
      <QuoteIssue
        quoteId={1}
        quotes={[{ id: 1, bike: 58, quote_status: QUOTE_ARCHIVED }]}
        quoteParts={[{ quote: 1, partType: 1 }, { quote: 1, partType: 22 }]}
        brands={sampleBrands}
        sections={sections}
        parts={[{ id: 15, partType: 1 }, { id: 25, partType: 1 }, { id: 35, partType: 22 }]}
    ...
    expect(component.find('QuoteSummaryParts')).toHaveLength(0);
  });
});
