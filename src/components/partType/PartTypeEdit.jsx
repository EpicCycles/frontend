...
class PartTypeEdit extends React.Component {
  handlePartTypeValueChange = (fieldName, input) => {
    const updatedPartType = processPartTypeValueChanges(
      this.props.partType,
  ...
  toggleDetail = () => {
    this.handlePartTypeValueChange(
      `detail_${this.props.componentKey}`,
      !this.props.partType._detail,
    );
  };
  addAnother = () => {
    const updatedPartType = updateObject(this.props.partType);
      ...
      <Fragment>
        <td>
          {componentKey !== NEW_ELEMENT_ID ? (
            ...
            <AddLink addFunction={this.addAnother} addObjectName={'Type'} />
          )}
        </td>
        <td>
          <PartTypeData
