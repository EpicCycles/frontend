...
class PartTypeSynonyms extends React.Component {
  handleInputChange = (fieldName, input) => {
    ...
    if (synonymToUpdateIndex > -1) {
      ...
      if (input) {
        ...
        synonymsWithUpdates[synonymToUpdateIndex].error_detail =
          'A value is required for the synonym';
      }
    ...
    if (synonymKey !== 'new') {
      ...
      if (synonymToUpdateIndex > -1) {
        if (window.confirm('Please confirm that you want to delete this Attribute')) {
          ...
          this.props.handlePartTypeChange(
            `synonyms_${this.props.partTypeKey}`,
            synonymsWithUpdates,
          );
        }
      }
  ...
  render() {
    ...
    const synonymsToUse = synonyms
      ? synonyms.filter(synonym => !(synonym.delete || synonym.dummyKey === NEW_ELEMENT_ID))
      : [];
    const newSynonym = synonyms
      ? synonyms.filter(synonym => synonym.dummyKey === NEW_ELEMENT_ID)
      : [];
      ...
      <Fragment>
        ...
        <div>
          <FormTextInput
            ...
            onClick={this.handleInputClear}
          />
          <AddLink addFunction={this.addAnother} addObjectName={'Synonym'} />
        </div>
      </Fragment>
    );
  }
