import { compareDatesDescending } from './sort';
...
describe('compareDatesDescending', () => {
  it('should return 0 when the dates are the same', () => {
    ...
    expect(compareDatesDescending(a, b)).toEqual(0);
  });
  ...
  it('should return positive when a is before b', () => {
    const a = new Date(2012, 11, 19, 3, 0, 0);
    const b = new Date(2012, 11, 20, 3, 0, 0);
    expect(compareDatesDescending(a, b) > 0).toBeTruthy();
  });
  it('should return negative when a is there but not b', () => {
    const a = new Date(2012, 11, 19, 3, 0, 0);
    const b = undefined;
    expect(compareDatesDescending(a, b) < 0).toBeTruthy();
  });
  it('should return positive when b is there but not a', () => {
    const a = undefined;
