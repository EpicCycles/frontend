...
export const quotePartValidation = (quotePart = {}, bikePart, partType, brands, parts, quote) => {
  ...
  if (quotePart.not_required) {
    ...
    if (!quotePart.trade_in_price) {
      if (bikePart && bikePart.trade_in_price) {
        validatedQuotePart.trade_in_price = bikePart.trade_in_price;
      } else {
        validatedQuotePart.error_detail.trade_in_price = 'Please specify a price (can be zero).';
      }
    }
  } else {
    if (quotePart.partType && !bikePart && !quotePart.part_desc) {
      validatedQuotePart.error_detail.part_desc = 'Please enter part details';
    }
  ...
  if (validatedQuotePart.part) {
    if (!quotePart.not_required && !validatedQuotePart.quantity) {
      validatedQuotePart.error_detail.quantity = 'Quantity is required for non replacement parts.';
    }
    if (isBikeQuote) {
      validatedQuotePart.ticket_price = undefined;
      validatedQuotePart.club_price = undefined;
      if (!validatedQuotePart.part_price)
        validatedQuotePart.error_detail.part_price = 'Please provide a price.';
    } else {
      if (validatedQuotePart.ticket_price) {
        validatedQuotePart.part_price = validatedQuotePart.ticket_price;
      } else {
        validatedQuotePart.error_detail.ticket_price = 'Please provide a ticket price.';
      }
