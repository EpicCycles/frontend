...
describe('buildSearchCriteria', () => {
  ...
  it('should add no criteria when only a token is passed', () => {
    expect(buildSearchCriteria({ token: 'hjfguygfieugfe' })).toBe('');
  });
  ...
  it('should add a single criteria when an object with one defined value is passed', () => {
    expect(buildSearchCriteria({ first_name: 'Fred', last_name: undefined })).toBe(
      '?first_name=Fred',
    );
  });
  it('should add multiple criteria when an object with many values is passed', () => {
    expect(buildSearchCriteria({ first_name: 'Fred', last_name: 'Jones', archived: true })).toBe(
      '?first_name=Fred&last_name=Jones&archived=true',
    );
  });
});
